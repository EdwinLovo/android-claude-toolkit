...
description: First-time setup for the small cross-cutting utility files — ErrorEventBus, UiText, ObserveAsEvents, OnLifecycleResumed, OnLifecycleEvent, Constants, StateFlowExt, the <PREVIEW> annotations + PreviewContainer, and the <ICONS> registry stub. Invoke when scaffolding these in a new project, or when a rule references any of them and the file doesn't exist yet.
allowed-tools:
  - Read
  - Write
  - Edit
...
The small cross-cutting utility files every project needs but that aren't big enough to warrant their own skill. Copy each into the paths below. Placeholder substitution applies throughout.

The shared UI component stubs (`<SCAFFOLD>`, `<DIALOG>`, `<PROJECT_NAME>AlertDialog`, `<PROJECT_NAME>ErrorAlertDialog`) live in the `component-primitives` skill.
...
Every rule that references `ErrorEventBus.send(...)`, `uiText.asString()`, `OnLifecycleResumed { }`, `<PREVIEW>Screen`, `<ICONS>.<Name>`, `ABSOLUTE_WEIGHT` / `EMPTY_STRING`, or `uiState.reduce { copy(...) }` will now resolve. See the corresponding rules linked at the top of this skill for usage patterns.
